...
ssh -t $master_instance "cd deep-neuroevolution && git pull && . scripts/run_master.sh nsr-es configurations/frostbite_nsres.json"

for worker in $worker_instances; do
	ssh -t $worker "pkill -u `whoami`"
	ssh -t $worker "cd deep-neuroevolution && git pull && . scripts/run_worker.sh nsr-es"
done
